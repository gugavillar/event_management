...
export const ParticipantsPayments = () => {
	...
	const { returnPayment, isPending: isReturnPending } = useReturnParticipantPayment()
	...
	const handleUpdate = useCallback(
		async (values: PaymentModalType) => {
			...
			const formatValues = {
				...
				paymentValue:
					values.paid === 'partial' && values.paymentValue
						...
						: Number(removeCurrencyFormat(selectedParticipant.event.participantPrice)),
			...
			await create(
				{ data: formatValues },
				{
					onError: (error) => generateToastError(error, 'Erro ao registrar pagamento do participante'),
					onSuccess: () => {
	...
	const handleReturnPayment = useCallback(async () => {
		...
		const results = await Promise.allSettled(selectedParticipant.payments.map((p) => returnPayment({ id: p.id })))
	...
	const paidValue = selectedParticipant?.payments?.reduce((total, p) => (total += Number(p.paymentValue)), 0) ?? 0
	...
	return (
		<PageContent pageTitle="Pagamentos dos participantes" subheadingPage="Lista de pagamentos">
			...
			>
				...
				{hasMoreThanOnePage && (
					<Pagination currentPage={page} setPage={setPage} totalPages={participantsPayments?.totalPages} />
				)}
			</ListPage>
			<PaymentModal
