...
export const Radio = forwardRef<HTMLInputElement, RadioProps>(
	({ options, isInvalid, position = 'column', fieldName, ...props }, ref) => {
		...
		return (
			<div className={twMerge('grid gap-2', position === 'column' ? 'space-y-3' : 'grid-cols-2')} data-testid="radio">
				{options?.map((option) => (
					<div className={twMerge('relative flex items-center', option.disabled && 'opacity-50')} key={option.value}>
