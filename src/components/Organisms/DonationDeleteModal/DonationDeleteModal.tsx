...
import type { SelectedDonation } from '@/components/Templates'
...
type DonationDeleteModalProps = {
	...
	setSelectedDonation: Dispatch<SetStateAction<SelectedDonation | null>>
}

export const DonationDeleteModal = memo(({ selectedDonation, setSelectedDonation }: DonationDeleteModalProps) => {
	const { remove, isPending } = useDeleteDonation()

	const handleDeleteDonation = async () => {
		if (!selectedDonation) return

		await remove(selectedDonation.id, {
	...
	return (
		<Modal onOpenChange={handleCloseDonationDeleteModal} open={selectedDonation?.modal === DONATION_MODAL_TYPE.DELETE}>
