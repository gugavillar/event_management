...
const handlerGet = async (request: NextRequest) => {
	...
	const isInterestedParams = Boolean(request.nextUrl.searchParams.get('isInterested'))
	const pageParams = Number(request.nextUrl.searchParams.get('pageParticipant')) || 1
...
const handlePost = async (request: NextRequest) => {
	...
	const body: ParticipantSchemaRouteType & { eventId: string } = await request.json()
...
export { handlePost as POST, handlerGet as GET }
